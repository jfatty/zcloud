...
layuiXtree.prototype.Loading = function (options) {
    ...
    $.ajax({
        ...
        data: options.params == null ? "" : JSON.stringify(options.params),
        dataType: 'json',
        beforeSend: function (xMLHttpRequest) {
            console.log("POST 树形数据在获取数据之前...");
        },
        success: function (value, type) {
            console.log(value.length);
            console.log(value);
            _this._dataJson = value.data; //将返回的数据转为json
            _this.Initial(options);
        },
...
//    obj.open('post', options.params, true);
...
//加载特效，且获取数据
layuiXtree.prototype.Get = function (options) {
    var _this = this;
    _this.xloading = document.createElement("span"); //创建加载对象
    _this.xloading.setAttribute('class', 'layui-icon layui-anim layui-anim-rotate layui-anim-loop');
    _this.xloading.innerHTML = '&#xe63e;';
