...
layui.define(['layer', 'laypage', 'laytpl'], function (exports) {
    ...
    var Paging = function () {
        this.config = {
            ...
            elem: undefined, //内容容器
            currPageIndex: undefined, //记录当前页码
            params: {}, //获取数据时传递的额外参数
     ...
     * 返回当前页码
     */
    Paging.prototype.getCurrPageIndex = function () {
        return this.config.currPageIndex;
    };
    /**
     * 初始化
     * @param {Object} options
    ...
    Paging.prototype.get = function (options) {
        ...
        var params = {};
        var contentType ;
        if("POST" === _config.type){
            Object.keys(_config.params).forEach(function(key){
                params[key] = _config.params[key] ;
            });
        ...
        $.ajax({
            type: _config.type,
            url: options.url == null ? _config.url : options.url,
            data: params,
            contentType : contentType,
            dataType: 'json',//返回的数据为json数据
            success: function (result, status, xhr) {
                ...
                if (result.rel) {
                    ...
                    if (_config.paged) {
                        ...
                        var defaults = {
                            elem: _pageConfig.elem, //指向存放分页的容器，值可以是容器ID、DOM对象。如： 1. elem: 'id' 注意：这里不能加 # 号 2. elem: document.getElementById('id')
                            curr: _config.params.pageIndex,//起始页。一般用于刷新类型的跳页以及HASH跳页
                            count: result.count,
                            limit: _pageConfig.pageSize,
                            jump: function (obj, first) {
                                //得到了当前页，用于向服务端请求对应数据
                                var curr = obj.curr;
                                _config.currPageIndex = curr;
                                if (options.refre) {//是否刷新页面
                                    options.refre = false;
                                    that.get({//调用数据获取函数
                ...
                if (_config.complate) {
                    ...
                    // $.ajax({
                    // 	type: "GET",
                    // 	url: _config.pageIndex,
                    // 	dataType: 'json',
                    // 	success: function (result, status, xhr) {
                    // 		var size = result.length;
                    // 		var perm = "" ;
                    // 		for(var i=0; i<size; i++){
                    // 			var privilege = result[i] ;
                    // 			perm += " " + privilege.sysPermission ;
                    // 		}
                    // 		$("a").removeClass(perm);
                    // 	}
                    // });
